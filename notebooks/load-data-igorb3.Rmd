...
For the analysis we will extract not more than 20K rows from EVENT table randomly using the HQL statement below. We skip the rows mising one of the folloowing important attributes: maker, model, mileage, manufacture_year or price_eur. As we can see, we had only 11431 samples complying to this condition:


```{r}
# Filters most important attributes
filter <- paste(
      " maker <> ''",
      " AND model <> ''",
...
# Filters the rest of the attributes
filter2 <- paste(
      " engine_displacement is not NULL",
...
count <- dbGetQuery(conn, paste("select count(*) from events", " WHERE", filter, " AND", filter2))
cars.sample.totalFilered <- count$`_c0`
```


```{r}
car.sample.maxSize <- 20000
